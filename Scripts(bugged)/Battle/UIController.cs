...
public class UIController : MonoBehaviour {
    ...
    private static LifeBarController hpBarPrefab;
                ...
                if (forcedaction == Actions.FIGHT) {
                    ...
                    for (int i = 0; i < encounter.EnabledEnemies.Length; i++) {
                        if (encounter.EnabledEnemies.Length > 3)
                            if (i > 1)
                                break;
                        LifeBarController lifebar = Instantiate(hpBarPrefab);
                        lifebar.player = true;
    ...
    private void Start() {
        LifeBarController lbc = Resources.Load<LifeBarController>("Prefabs/HPBar");
        if (lbc != null && lbc.ToString().ToLower() != "null")
            hpBarPrefab = lbc;
