...
public class NewMusicManager {
    ...
    public static void CreateChannel(string name) {
        if (audiolist.ContainsKey(name)) throw new InvalidOperationException("The current audio channel already exists.");
    ...
    public static void DestroyChannel(string name) {
        if (name == "src")                throw new InvalidOperationException("You can't delete the audio channel \"src\".");
