...
function Update()
    ...
    if spawntimer%60 == 0 then
        ...
        money.SetVar("velX",-2 * lr) -- x-speed for the bullet is either -1 (if spawned on right side) or 1 (if spawned on left side)
        money.SetVar("velY",0) -- no y-speed yet
		money.SetVar("CollGround",math.random(4)-1) -- didnt collide with the ground yet
    ...
    for i=1,#Moneys do
		...
		if money.y < -Arena.height/2 + 8 then -- if it goes below the arena
			...
			if CollGround < 4 then -- if it has collided less than 4 times w/ the ground
				velY = 4 - CollGround -- that way it bounces less every time it collides with the ground
			else
